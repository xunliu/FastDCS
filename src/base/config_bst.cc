...
void** bst_probe (struct bst_table *tree, void *item) {
  ...
  int dir = 0;                // Side of |q| on which |p| is located.
...
void* bst_replace (struct bst_table *table, void *item) {
  void **p = bst_probe (table, item);
  if (p == NULL || *p == item)
    return NULL;
  else {
...
void* bst_t_insert (struct bst_traverser *trav, struct bst_table *tree, void *item) {
  ...
  trav->bst_node = *q = (struct bst_node *)tree->bst_alloc->libavl_malloc (tree->bst_alloc, sizeof **q);
  if (*q == NULL) {
    trav->bst_node = NULL;
    trav->bst_generation = tree->bst_generation;
    return NULL;
...
void* bst_t_replace (struct bst_traverser *trav, void *new_item) {
  void *old;

  assert (trav != NULL && trav->bst_node != NULL && new_item != NULL);
  old = trav->bst_node->bst_data;
  trav->bst_node->bst_data = new_item;
  return old;
}

static void copy_error_recovery (struct bst_node **stack, int height,
                     struct bst_table *new_item, bst_item_func *destroy) {
  assert (stack != NULL && height >= 0 && new_item != NULL);

  for (; height > 2; height -= 2)
    stack[height - 1]->bst_link[1] = NULL;

  bst_destroy (new_item, destroy);
}
...
struct bst_table* bst_copy (const struct bst_table *org, bst_copy_func *copy,
  ...
  struct bst_table *new_item;
  const struct bst_node *x;
  struct bst_node *y;

  assert (org != NULL);
  new_item = bst_create (org->bst_compare, org->bst_param,
                    allocator != NULL ? allocator : org->bst_alloc);
  if (new_item == NULL)
    return NULL;
  new_item->bst_count = org->bst_count;
  if (new_item->bst_count == 0)
    return new_item;

  x = (const struct bst_node *) &org->bst_root;
  y = (struct bst_node *) &new_item->bst_root;
  for (;;) {
    while (x->bst_link[0] != NULL) {
      if (height >= 2 * (BST_MAX_HEIGHT + 1)) {
        y->bst_data = NULL;
        y->bst_link[0] = y->bst_link[1] = NULL;
        copy_error_recovery (stack, height, new_item, destroy);
      ...
      y->bst_link[0] =
        (struct bst_node*)new_item->bst_alloc->libavl_malloc (new_item->bst_alloc,
                                                              sizeof *y->bst_link[0]);
      if (y->bst_link[0] == NULL) {
        if (y != (struct bst_node *) &new_item->bst_root) {
          y->bst_data = NULL;
          y->bst_link[1] = NULL;
        }
...
void* bst_malloc (struct libavl_allocator *allocator, size_t size) {
  assert (allocator != NULL && size > 0);
  return malloc (size);
}

// Frees |block|.
void bst_free (struct libavl_allocator *allocator, void *block) {
...
void (bst_assert_insert) (struct bst_table *table, void *item) {
  void **p = bst_probe (table, item);
...
void * (bst_assert_delete) (struct bst_table *table, void *item) {
